...
% Non SD parameters
% -------------------------------------------------------------------------
alpha     = .17; %param(1);  % share of capital in the prod. function
iotap     = .05;%param(2);  % price indexation (=0 static indexation, =1 dynamic)
iotaw     = .08;%param(3);  % wages indexation
gamma100  = 0;%param(4);  % steady state growth rate of technology (multiplied by 100)
h         = .84;%param(5);  % habit formation
lambdapss = .29;%param(6);  % steady state net price markup
lambdawss = .29;% param(7);  % steady state net wage markup
Lss       = param(8);  % steady state for log hours
pss100    = 0;%param(9);  % steady state quarterly net inflation (multiplied by 100)
Fbeta     = param(10); % 100 * ( inv(discount factor) - 1 )
niu       = 1.96;%param(11); % inverse Frisch elasticity
xip       = .77;%param(12); % price stickiness
xiw       = .73;%param(13); % wage stickiness
chi       = 4.95;%param(14); % elasticity of the capital utilization cost function
S         = 3.58;%param(15); % investment adjustment cost
fp        = 2.1;%param(16); % reaction to inflation
fy        = 0;%.94;%param(17); % reaction to output gap
fdy       = 0;%param(18); % reaction to output gap growth

rhoR = .78;%param(19); 
rhoz = param(20); rhog = param(21); rhomiu = param(22); 
rholambdap = param(23); rholambdaw = param(24); rhob = param(25); rhomp = .2;%param(26);
rhoARMAlambdap = param(27); rhoARMAlambdaw = param(28);

% Standard deviations
sdR = 1;%param(29); 
sdz = param(30); sdg = param(31); sdmiu = param(32); 
...
tau_D       = 0;%param(39); % Where all the cases come in
tau_k       = 0;%param(40);
...
% -------------------------------------------------------------------------
gamma  = 0;%gamma100 / 100;
expg   = exp(gamma);
beta   = .9988;% 100 / (Fbeta + 100);
...
% ===
GAM0(kbar_s_star, kbar_s_star) = GAM0(kbar_s, kbar_s);
GAM0(kbar_s_star, miu)         = GAM0(kbar_s, miu);
GAM0(kbar_s_star, i_s_star)    = GAM0(kbar_s, i_s);
GAM1(kbar_s_star, kbar_s_star) = GAM1(kbar_s, kbar_s);
GAM0(kbar_s_star, z)           = GAM0(kbar_s, z);
% ===
...
% eq 25 and 26, wage gap (wgap and wgapstar) TODO: normalization!
% -------------------------------------------------------------------------
GAM0(wgap, wgap)   = 1;
GAM0(wgap, w)      = -1;
