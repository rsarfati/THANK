...
% New variables to define inequality and risk
GAMMA       = 45;
v           = 46;
eGAMMA      = 47;

% New variables to define the disturbances
MPshock     = 48;
TECHshock     = 49;
Gshock     = 50;
Ishock     = 51;
...
GAMMAss  = yss/c_h_ss/(1-theta)-theta/(1-theta);
vss      = sigma*(1-sigma)*log(GAMMAss)^2;
...
% New equations to define inequality and risk
% -------------------------------------------------------------------------
GAM0(GAMMA,GAMMA)=1;
GAM0(GAMMA,y)=-yss/c_h_ss/(1-theta)/GAMMAss;
GAM0(GAMMA,c_h)=yss/c_h_ss/(1-theta)/GAMMAss;

GAM0(eGAMMA,GAMMA)=1; GAM1(eGAMMA,eGAMMA)=1; PPI(eGAMMA,GAMMAex)=1;

GAM0(v,v)=1;
GAM0(v,ey)=sigma_prime*sigma*(2*sigma-1);
GAM0(v,eGAMMA)=-2*sigma*(1-sigma)*log(GAMMAss);

% New equations to define the disturbances
% -------------------------------------------------------------------------
GAM0(MPshock,MPshock)=1; PSI(MPshock, Rs) = 1;
GAM0(TECHshock,TECHshock)=1; PSI(TECHshock, zs) = 1;
GAM0(Gshock,Gshock)=1; PSI(Gshock, gs) = 1;
GAM0(Ishock,Ishock)=1; PSI(Ishock, mius) = 1;
GAM0(MKPshock,MKPshock)=1; PSI(MKPshock, lambdaps) = 1;
GAM0(MKWshock,MKWshock)=1; PSI(MKWshock, lambdaws) = 1;
GAM0(PREFshock,PREFshock)=1; PSI(PREFshock, bs) = 1;
